...
사이트 접속 : 전체 접속자 수 
1. application.setAttribute("count",0); //변수명 count , 값 : 0
...
A라는 사용자 웹 서버 접속
서버 session 객체 생성 > 식별값 > 접속 브라우저(response)
session.setAttribut("userid", "kglim");

B라는 사용자 웹 서버 접속
서버 session 객체 생성 > 식별값 > 접속 브라우저(response)
session.setAttribut("userid", "hong");

Application 변수는 사용자마다 같은 값
Session변수는 접속하는 사용자마다 다른 값을 가질 수 있다 .
-->
<h3>세션정</h3>
